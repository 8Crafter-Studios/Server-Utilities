          ...
          helper.registerRule((id) => {
            ...
            if (currentDesc.match) {
              return new MatchRule(
                ...
                currentDesc.name,
                currentDesc.match,
                _RuleFactory._compileCaptures(currentDesc.captures, helper, repository)
              );
            }
            ...
            if (currentDesc.while) {
              return new BeginWhileRule(
                ...
                currentDesc.contentName,
                currentDesc.begin,
                _RuleFactory._compileCaptures(currentDesc.beginCaptures || currentDesc.captures, helper, repository),
                currentDesc.while,
                _RuleFactory._compileCaptures(currentDesc.whileCaptures || currentDesc.captures, helper, repository),
            ...
            return new BeginEndRule(
              ...
              _RuleFactory._compileCaptures(currentDesc.beginCaptures || currentDesc.captures, helper, repository),
              currentDesc.end,
              _RuleFactory._compileCaptures(currentDesc.endCaptures || currentDesc.captures, helper, repository),
            ...
            for (const pattern of patterns) {
              ...
              if (pattern.include) {
                ...
                switch (reference.kind) {
                  case 0:
                  case 1:
                    ruleId = memo[pattern.include] || (memo[pattern.include] = _RuleFactory.getCompiledRuleId(repository[pattern.include], helper, repository));
                    break;
                  case 2:
                    let localIncludedRule = repository[reference.ruleName];
                    if (localIncludedRule) {
                      ruleId = memo[reference.ruleName] || (memo[reference.ruleName] = _RuleFactory.getCompiledRuleId(localIncludedRule, helper, repository));
                    } else {
                    }
                    break;
                  case 3:
                  case 4:
                    ...
                    if (externalGrammar) {
                      if (externalGrammarInclude) {
                        let externalIncludedRule = externalGrammar.repository[externalGrammarInclude];
                        if (externalIncludedRule) {
                          ruleId = memo[externalGrammarInclude] || (memo[externalGrammarInclude] = _RuleFactory.getCompiledRuleId(externalIncludedRule, helper, externalGrammar.repository));
                        } else {
                        }
                      } else {
                        ruleId = memo[externalGrammarName] || (memo[externalGrammarName] = _RuleFactory.getCompiledRuleId(externalGrammar.repository.$self, helper, externalGrammar.repository));
                      }
                    } else {
                    }
                    break;
                ...
                ruleId = memo[pattern.name] || (memo[pattern.name] = _RuleFactory.getCompiledRuleId(pattern, helper, repository));
              }
              if (ruleId !== -1) {
                const rule = helper.getRule(ruleId);
                let skipRule = false;
                      ...
                      case 2:
                        ...
                        if (localIncludedRule) {
                          if (!memo[reference.ruleName]) {
                            ...
                            memo[reference.ruleName] = _RuleFactory.getCompiledRuleId(localIncludedRule, helper, repository);
                          }
                          ...
                          if (externalGrammarInclude) {
                            ...
                            if (externalIncludedRule) {
                              if (!memo[externalGrammarInclude]) {
                                memo[externalGrammarInclude] = _RuleFactory.getCompiledRuleId(externalIncludedRule, helper, externalGrammar.repository);
                              }
                            ...
                            if (!memo[externalGrammarName]) {
                              memo[externalGrammarName] = _RuleFactory.getCompiledRuleId(externalGrammar.repository.$self, helper, externalGrammar.repository);
                            }
