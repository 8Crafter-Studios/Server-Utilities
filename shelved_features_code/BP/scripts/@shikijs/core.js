const exports = (() => {
  /**
   * This is an empty async function used to prevent stack overflow errors;
   */
  async function preventStackOverflowError(){}
  ...
  function collectExternalReferencesInRules(rules, context, result) {
    ...
    while (queue.length > 0) {
      ...
      const patternRepository = rule.repository ? mergeObjects({}, context.repository, rule.repository) : context.repository;
      if (Array.isArray(rule.patterns)) {
        queue.push(...rule.patterns.map(pattern => ({ ...context, repository: patternRepository, rule: pattern })));
      }
      const include = rule.include;
      if (!include) {
        continue;
      ...
      switch (reference.kind) {
        case 0:
          queue.push({ ...context, selfGrammar: context.baseGrammar });
          break;
        case 1:
          queue.push(context);
          break;
        case 2:
          queue.push({ ...context, repository: patternRepository, ruleName: reference.ruleName });
          break;
        case 3:
        case 4:
          const selfGrammar = reference.scopeName === context.selfGrammar.scopeName ? context.selfGrammar : reference.scopeName === context.baseGrammar.scopeName ? context.baseGrammar : void 0;
          if (selfGrammar) {
            const newContext = { baseGrammar: context.baseGrammar, selfGrammar, repository: patternRepository };
            if (reference.kind === 4) {
              queue.push({ ...newContext, ruleName: reference.ruleName });
            } else {
  ...
  var RuleFactory = class _RuleFactory {/* 
    ...
    static async createCaptureRule(helper, $location, name, contentName, retokenizeCapturedWithRuleId) {
      return await helper.registerRule((id) => {
        return new CaptureRule($location, id, name, contentName, retokenizeCapturedWithRuleId);
      });
    }/* 
    static async getCompiledRuleId(desc, helper, repository) {
      if (!desc.id) {
        helper.registerRule((id) => {
          ...
          if (desc.match) {
            return new MatchRule(
              ...
              desc.name,
              desc.match,
              await _RuleFactory.getCompiledRuleId(desc.captures, helper, repository)
            );
          }
          ...
          if (desc.while) {
            return new BeginWhileRule(
              ...
              desc.contentName,
              desc.begin,
              await _RuleFactory.getCompiledRuleId(desc.beginCaptures || desc.captures, helper, repository),
              desc.while,
              await _RuleFactory.getCompiledRuleId(desc.whileCaptures || desc.captures, helper, repository),
          ...
          return new BeginEndRule(
            ...
            await _RuleFactory.getCompiledRuleId(desc.beginCaptures || desc.captures, helper, repository),
            desc.end,
            await _RuleFactory.getCompiledRuleId(desc.endCaptures || desc.captures, helper, repository),
    ...
    static async getCompiledRuleId(desc, helper, repository) {
      await preventStackOverflowError();
      if (!desc.id) {
        const queue = [desc];
        while (queue.length > 0) {
          const currentDesc = queue.shift();
          await helper.registerRule(async (id) => {
            currentDesc.id = id;
            if (currentDesc.match) {
              return new MatchRule(
                currentDesc.$vscodeTextmateLocation,
                currentDesc.id,
                currentDesc.name,
                currentDesc.match,
                await _RuleFactory.getCompiledRuleId(currentDesc.captures, helper, repository)
              );
            }
            if (typeof currentDesc.begin === "undefined") {
              if (currentDesc.repository) {
                repository = mergeObjects({}, repository, currentDesc.repository);
              }
              let patterns = currentDesc.patterns;
              if (typeof patterns === "undefined" && currentDesc.include) {
                patterns = [{ include: currentDesc.include }];
              }
              return new IncludeOnlyRule(
                currentDesc.$vscodeTextmateLocation,
                currentDesc.id,
                currentDesc.name,
                currentDesc.contentName,
                _RuleFactory._compilePatterns(patterns, helper, repository)
              );
            }
            if (currentDesc.while) {
              return new BeginWhileRule(
                currentDesc.$vscodeTextmateLocation,
                currentDesc.id,
                currentDesc.name,
                currentDesc.contentName,
                currentDesc.begin,
                await _RuleFactory.getCompiledRuleId(currentDesc.beginCaptures || currentDesc.captures, helper, repository),
                currentDesc.while,
                await _RuleFactory.getCompiledRuleId(currentDesc.whileCaptures || currentDesc.captures, helper, repository),
                _RuleFactory._compilePatterns(currentDesc.patterns, helper, repository)
              );
            }
            return new BeginEndRule(
              currentDesc.$vscodeTextmateLocation,
              ...
              await _RuleFactory.getCompiledRuleId(currentDesc.beginCaptures || currentDesc.captures, helper, repository),
              currentDesc.end,
              await _RuleFactory.getCompiledRuleId(currentDesc.endCaptures || currentDesc.captures, helper, repository),
              currentDesc.applyEndPatternLast,
              _RuleFactory._compilePatterns(currentDesc.patterns, helper, repository)
            );
          });
        }
    ...
    static async _compileCaptures(captures, helper, repository) {
      let r = [];
      if (captures) {
        ...
        for (const captureId in captures) {
          ...
          if (captures[captureId].patterns) {
            retokenizeCapturedWithRuleId = await _RuleFactory.getCompiledRuleId(captures[captureId], helper, repository);
          }
    ...
    static async _compileCaptures(captures, helper, repository) {
      ...
      if (captures) {
        ...
        for (const captureId in captures) {
          ...
          queue.push({ captureId, numericCaptureId });
        }
        ...
        while (queue.length > 0) {
          const { captureId, numericCaptureId } = queue.shift();
          let retokenizeCapturedWithRuleId = 0;
          if (captures[captureId].patterns) {
            retokenizeCapturedWithRuleId = await _RuleFactory.getCompiledRuleId(captures[captureId], helper, repository);
          }
          result[numericCaptureId] = _RuleFactory.createCaptureRule(helper, captures[captureId].$vscodeTextmateLocation, captures[captureId].name, captures[captureId].contentName, retokenizeCapturedWithRuleId);
        }
      }
      return result;
    } */
    static async _compileCaptures(captures, helper, repository) {
      ...
      if (captures) {
        ...
        for (let i = 0; i <= Math.max(...queue.map(({ numericCaptureId }) => numericCaptureId)); i++) {
          result[i] = null;
        }
        ...
        while (queue.length > 0) {
          ...
          result[numericCaptureId] = await _RuleFactory.createCaptureRule(helper, captures[captureId].$vscodeTextmateLocation, captures[captureId].name, captures[captureId].contentName, retokenizeCapturedWithRuleId);
        }
      }
      return result;
    }
    /* static async _compilePatterns(patterns, helper, repository) {
      let r = [];
      if (patterns) {
        for (let i = 0, len = patterns.length; i < len; i++) {
          ...
          if (pattern.include) {
            ...
            switch (reference.kind) {
              case 0:
              case 1:
                ruleId = await _RuleFactory.getCompiledRuleId(repository[pattern.include], helper, repository);
                break;
              case 2:
                let localIncludedRule = repository[reference.ruleName];
                if (localIncludedRule) {
                  ruleId = await _RuleFactory.getCompiledRuleId(localIncludedRule, helper, repository);
                } else {
                }
                break;
              case 3:
                ...
                if (externalGrammar) {
                  if (externalGrammarInclude) {
                    ...
                    if (externalIncludedRule) {
                      ruleId = await _RuleFactory.getCompiledRuleId(externalIncludedRule, helper, externalGrammar.repository);
                    } else {
                    }
                  } else {
                    ruleId = await _RuleFactory.getCompiledRuleId(externalGrammar.repository.$self, helper, externalGrammar.repository);
                  }
                } else {
                }
                break;
            ...
            ruleId = await _RuleFactory.getCompiledRuleId(pattern, helper, repository);
          }
          if (ruleId !== -1) {
      ...
      static async _compilePatterns(patterns, helper, repository) {
        const memo = {};
        const result = [];
        console.log(patterns instanceof Array);
        const stack = [patterns];
        while (stack.length > 0) {
          const patterns = stack.pop();
            ...
            for (const pattern of patterns) {
              ...
              if (pattern.include) {
                ...
                switch (reference.kind) {
                  case 0:
                  case 1:
                    ruleId = memo[pattern.include] || (memo[pattern.include] = await _RuleFactory.getCompiledRuleId(repository[pattern.include], helper, repository));
                    break;
                  case 2:
                    let localIncludedRule = repository[reference.ruleName];
                    if (localIncludedRule) {
                      ruleId = memo[reference.ruleName] || (memo[reference.ruleName] = await _RuleFactory.getCompiledRuleId(localIncludedRule, helper, repository));
                    } else {
                    }
                    break;
                  case 3:
                  case 4:
                    ...
                    if (externalGrammar) {
                      if (externalGrammarInclude) {
                        let externalIncludedRule = externalGrammar.repository[externalGrammarInclude];
                        if (externalIncludedRule) {
                          ruleId = memo[externalGrammarInclude] || (memo[externalGrammarInclude] = await _RuleFactory.getCompiledRuleId(externalIncludedRule, helper, externalGrammar.repository));
                        } else {
                        }
                      } else {
                        ruleId = memo[externalGrammarName] || (memo[externalGrammarName] = await _RuleFactory.getCompiledRuleId(externalGrammar.repository.$self, helper, externalGrammar.repository));
                      }
                    } else {
                    }
                    break;
                ...
                ruleId = memo[pattern.name] || (memo[pattern.name] = await _RuleFactory.getCompiledRuleId(pattern, helper, repository));
              }
              if (ruleId !== -1) {
                const rule = helper.getRule(ruleId);
                let skipRule = false;
        ...
        return {
          patterns: result,
          hasMissingPatterns: (patterns ? patterns.length : 0) !== result.length
        };
      } *//* 
        static async _compilePatterns(patterns, helper, repository) {
          const memo = {};
          const result = [];
          const stack = [{ patterns, result }];
        
          while (stack.length > 0) {
            const { patterns: currentPatterns, result: currentResult } = stack.pop();
            ...
            if (currentPatterns && currentPatterns.length > 0) {
              const newPatterns = currentPatterns.slice(1); // Get the remaining patterns
              stack.push({ patterns: newPatterns, result: currentResult });
            }
          }
            ...
            while (stack.length > 0) {
              ...
              if (Array.isArray(patterns)) {
                for (const pattern of patterns) {
                  ...
                  if (pattern.include) {
                    ...
                    switch (reference.kind) {
                      ...
                      case 2:
                        let localIncludedRule = repository[reference.ruleName];
                        if (localIncludedRule) {
                          if (!memo[reference.ruleName]) {
                            iiii++;
                            console.log("a", iiii, new Error().stack.split(/\n\s*at/g).length);
                            memo[reference.ruleName] = await _RuleFactory.getCompiledRuleId(localIncludedRule, helper, repository);
                          }
                          ruleId = memo[reference.ruleName];
                        } else {
                        }
                        break;
                      case 3:
                      case 4:
                        ...
                        if (externalGrammar) {
                          if (externalGrammarInclude) {
                            let externalIncludedRule = externalGrammar.repository[externalGrammarInclude];
                            if (externalIncludedRule) {
                              if (!memo[externalGrammarInclude]) {
                                memo[externalGrammarInclude] = await _RuleFactory.getCompiledRuleId(externalIncludedRule, helper, externalGrammar.repository);
                              }
                              ruleId = memo[externalGrammarInclude];
                            } else {
                            }
                          } else {
                            if (!memo[externalGrammarName]) {
                              memo[externalGrammarName] = await _RuleFactory.getCompiledRuleId(externalGrammar.repository.$self, helper, externalGrammar.repository);
                            }
                            ruleId = memo[externalGrammarName];
                          }
                        } else {
                        }
                        break;
                  ...
                  if (ruleId !== -1) {
                    const rule = await helper.getRule(ruleId);
                    let skipRule = false;
                    if (rule instanceof IncludeOnlyRule || rule instanceof BeginEndRule || rule instanceof BeginWhileRule) {
                      if (rule.hasMissingPatterns && rule.patterns.length === 0) {
                        skipRule = true;
  ...
  async function _tokenizeString(grammar, lineText, isFirstLine, linePos, stack, lineTokens, checkWhileConditions, timeLimit) {
    const lineLength = lineText.content.length;
    let STOP = false;
    let anchorPosition = -1;
    if (checkWhileConditions) {
      const whileCheckResult = await _checkWhileConditions(
        grammar,
        lineText,
        isFirstLine,
        linePos,
    ...
    async function scanNext() {
      ...
      console.log("scanNext stack: ", stack)
      const r = await matchRuleOrInjections(
        grammar,
        lineText,
        isFirstLine,
        linePos,
      ...
      if (matchedRuleId === endRuleId) {
        const poppedRule = await stack.getRule(grammar);
        if (false) {
          console.log(
  ...
  async function _checkWhileConditions(grammar, lineText, isFirstLine, linePos, stack, lineTokens) {
    let anchorPosition = stack.beginRuleCapturedEOL ? 0 : -1;
    const whileRules = [];
    for (let node = stack; node; node = node.pop()) {
      const nodeRule = await node.getRule(grammar);
  ...
  async function matchRuleOrInjections(grammar, lineText, isFirstLine, linePos, stack, anchorPosition) {
    const matchResult = await matchRule(grammar, lineText, isFirstLine, linePos, stack, anchorPosition);
  ...
  async function matchRule(grammar, lineText, isFirstLine, linePos, stack, anchorPosition) {
    if (!stack) {
      console.error("stack is missing at matchRule:", grammar, lineText, isFirstLine, linePos, stack, anchorPosition, new Error().stack);
      return null;
    }
    const rule = await stack.getRule(grammar);
  ...
  var Grammar = class {
    ...
    async registerRule(factory) {
      const id = ++this._lastRuleId;
      const result = await factory(ruleIdFromNumber(id));
    ...
    async tokenizeLine(lineText, prevState, timeLimit = 0) {
      const r = await this._tokenize(lineText, prevState, false, timeLimit);
      return {
    ...
    async tokenizeLine2(lineText, prevState, timeLimit = 0) {
      const r = await this._tokenize(lineText, prevState, true, timeLimit);
      return {
    ...
    async _tokenize(lineText, prevState, emitBinaryTokens, timeLimit) {
      if (this._rootId === -1) {
        this._rootId = await RuleFactory.getCompiledRuleId(
...
\f\r "&'/<=>`.split(""), "\0	\n\f\r \"&'/<=>`".split("")]
  ...
  async function codeToTokensBase(internal, code, options = {}) {
    const {
      lang = "text",
    ...
    return await tokenizeWithTheme(code, _grammar, theme, colorMap, options);
  }
  async function getLastGrammarState(...args) {
    if (args.length === 2) {
    ...
    return new GrammarState(
      await _tokenizeWithTheme(code, _grammar, theme, colorMap, options).stateStack,
      _grammar.name,
      theme.name
    );
  }
  async function tokenizeWithTheme(code, grammar, theme, colorMap, options) {
    const result = await _tokenizeWithTheme(code, grammar, theme, colorMap, options);
    const grammarState = new GrammarState(
      (await _tokenizeWithTheme(code, grammar, theme, colorMap, options)).stateStack,
      grammar.name,
      theme.name
    );
  ...
  async function _tokenizeWithTheme(code, grammar, theme, colorMap, options) {
    const colorReplacements = resolveColorReplacements(theme, options);
    const {
    ...
    let stateStack = options.grammarState ? getGrammarStack(options.grammarState, theme.name) ?? INITIAL : options.grammarContextCode != null ? (await _tokenizeWithTheme(
    ...
    for (let i = 0, len = lines.length; i < len; i++) {
      ...
      if (options.includeExplanation) {
        resultWithScopes = await grammar.tokenizeLine(line, stateStack, tokenizeTimeLimit);
        tokensWithScopes = resultWithScopes.tokens;
        tokensWithScopesIndex = 0;
      }
      const result = await grammar.tokenizeLine2(line, stateStack, tokenizeTimeLimit);
  ...
  async function codeToTokensWithThemes(internal, code, options) {
    const themes = Object.entries(options.themes).filter((i) => i[1]).map((i) => ({ color: i[0], theme: i[1] }));
    const themedTokens = [];
    for (const t of themes.map(async () => {
      const tokens2 = await codeToTokensBase(internal, code, {
        ...options,
        theme: t.theme
      })
  ...
  async function codeToTokens(internal, code, options) {
    let bg;
    let fg;
    let tokens;
    let themeName;
    ...
    if ("themes" in options) {
      ...
      const themeTokens = await codeToTokensWithThemes(
        internal,
        code,
        options
      );
  ...
  async function codeToHast(internal, code, options, transformerContext = {
    meta: {},
    options,
    ...
    let {
      ...
      rootStyle,
      grammarState
    } = await codeToTokens(internal, input, options);
    const {
  ...
  async function codeToHtml(internal, code, options) {
    const context = {
      meta: {},
      options,
      codeToHast: (_code, _options) => codeToHast(internal, _code, _options),
  ...
  async function createHighlighterCore(options) {
    ...
    return {
      getLastGrammarState: async (...args) => await getLastGrammarState(internal, ...args),
  ...
  function createSingletonShorthands(createHighlighter, config) {
    ...
    return {
      ...
      async getLastGrammarState(code, options) {
        ...
        return await shiki.getLastGrammarState(code, options);
      }
    };
  }
