...
export function manageCommands(sourceEntity: Entity|Player){
    ...
    forceShow(form, sourceEntity as Player).then(ra=>{
        ...
        switch(r.selection){
            ...
            let form5 = new ModalFormData; form5.title(`Add Custom Command`); form5.textField("Command Name", "myCommand"); form5.dropdown("Command Code Type", ["commands", "javascript"]); form5.textField("Command Version", "SemVer String; ex. 1.7.0-beta.1.2.3.4.a.b.c.d", "1.0.0"); form5.textField("Formatting Code", "§§r§§f", "§r§f"); form5.textField("Description", "String"); form5.textField("formats", "Array", "[\"myCommand\", \"myCommand <string: string> [integer: int]\"]")
            forceShow(form5, sourceEntity as Player).then(ha=>{let h = (ha as ModalFormResponse); 
                if(h.canceled){return};
                new command({commandName: String(h.formValues[0]), commands_format_version: commands_format_version, command_version: String(h.formValues[2]), customCommandType: ["commands", "javascript"][Number(h.formValues[1])] as "commands" | "javascript", description: String(h.formValues[4]), type: "custom", formatting_code: String(h.formValues[3]), formats: (sourceEntity as Player)?.name??sourceEntity?.nameTag, format_version: format_version}).save()
                manageCommands(sourceEntity)
            }).catch((e)=>{let formError = new MessageFormData; formError.body(e+e.stack); formError.title("Error"); formError.button1("Done"); forceShow(formError, sourceEntity as Player).then(()=>{return e}); }); 
            break
            ...
            forceShow(form2, sourceEntity as Player).then(ga=>{let g = (ga as ActionFormResponse); 
                ...
                switch(g.selection+Number(commandsItem.type!="custom")){
                    ...
                    break
                    case 1: 
                    let form5 = new ModalFormData; form5.title(`Edit Custom Command`); form5.textField("Command Name", "myCommand", commandsItem.commandName); form5.textField("", "Decimal"); form5.textField("Reason", "JavaScript Object ex. `\nDate: ${new Date(D\nate\n.now()).toLo\ncaleString()}`", "\"§cYOU HAVE BEEN BANNED BY THE BAN HAMMER\\nBanned By: {bannedByName}\\nBanned Until: {unbanDate}\\nBanned On: {banDate}\\nTime Remaining: {timeRemaining}\"")
                    forceShow(form5, sourceEntity as Player).then(ha=>{let h = (ha as ModalFormResponse); 
                        if(h.canceled){return};
                        ban.saveBan({removeAfterBanExpires: false, ban_format_version: ban_format_version, banDate: Date.now(), playerId: String(h.formValues[0]), originalPlayerName: undefined, type: "id", bannedById: sourceEntity.id, bannedByName: (sourceEntity as Player)?.name??sourceEntity?.nameTag, banId: "banId:"+Date.now()+":"+String(h.formValues[0]), unbanDate: Number(h.formValues[1])*60000+Date.now(), format_version: format_version, reason: String(h.formValues[2])})
                        manageCommands(sourceEntity)
                    ...
                    break
                    case 3: 
                    let form6 = new ModalFormData; form6.title(`Command Settings`); form6.textField("Command Name", JSONStringify(commandsItem.settings?.defaultSettings?.requiredTags??["canUseChatCommands", "canUseCustomCommands"]), JSONStringify(commandsItem.settings.requiredTags)); form6.textField("", "Decimal"); form6.textField("Reason", "JavaScript Object ex. `\nDate: ${new Date(D\nate\n.now()).toLo\ncaleString()}`", "\"§cYOU HAVE BEEN BANNED BY THE BAN HAMMER\\nBanned By: {bannedByName}\\nBanned Until: {unbanDate}\\nBanned On: {banDate}\\nTime Remaining: {timeRemaining}\"")
                    forceShow(form6, sourceEntity as Player).then(ha=>{let h = (ha as ModalFormResponse); 
                        if(h.canceled){return};
                        ban.saveBan({removeAfterBanExpires: false, ban_format_version: ban_format_version, banDate: Date.now(), playerId: String(h.formValues[0]), originalPlayerName: undefined, type: "id", bannedById: sourceEntity.id, bannedByName: (sourceEntity as Player)?.name??sourceEntity?.nameTag, banId: "banId:"+Date.now()+":"+String(h.formValues[0]), unbanDate: Number(h.formValues[1])*60000+Date.now(), format_version: format_version, reason: String(h.formValues[2])})
                        manageCommands(sourceEntity)
