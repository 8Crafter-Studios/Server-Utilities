...
export const commands = [
    ...
    {type: "built-in", requiredTags: ["canUseChatCommands"], formatting_code: "§r§f", commandName: "binvsee", escregexp: {v: "^binvsee$"}, formats: [{format: "binvsee <dimension: dimensionId|~> <blockLocation: x y z>"}], command_version: "1.6.0", description: "Invsees into a block. ", commandSettingsId: "built-inCommandSettings:binvsee"}, 
...
export const JunkItemTypes = ["dirt", "stick", "dead_bush", "tripwire_hook", "rotten_flesh", "string", "cobblestone", "stone", "dorite", "andesite", "granite", "tuff", "end_stone", "seeds", "tall_grass", "fern"]
...
export const IllegalItemTypes = ["netherreactor", "glowingobsidian", "stonecutter", "water", "flowing_water", "lava", "flowing_lava", "camera", "item.camera", "item.door", "item.skull", "item.cauldron", "bedrock"]
    ...
    try{let slotsArray = []; let players = savedPlayer.getSavedPlayers().filter((p)=>(p.name==switchTestB.split(" ").slice(1).join(" "))); if(players.length==0){player.sendMessage("§cError: no players with that name were found")}else{if(players.length>1){player.sendMessage("§cError: multiple saved players with that name were found, with the following uuids: "+[players[0]?.id, players[1]?.id, players[2]?.id, players[3]?.id])}else{let player = players[0]; let items = player.items.inventory.concat(player.items.equipment); items.forEach((item)=>{if (item.count != 0) {slotsArray = slotsArray.concat(String("slot: " + item.slot + "§r§f, item: " + item.id + "§r§f, amount: " + item.count + "§r§f, nameTag: " + item.name + "§r§f, lore: " + JSONStringify(item.lore??[], true)??"[]" + "§r§f, enchantments: " + JSONStringify(item.enchants??"N/A", true)??"N/A"))}else{slotsArray = slotsArray.concat("slot: " + item.slot + ", item: minecraft:air")}}); ; eventData.sender.sendMessage(String("(format_version: " + player.format_version + ") " + player.name + (world.getAllPlayers().find((p)=>(p.id==player.id))!=undefined?" (Online)":" (last seen: "+new Date(Number(player.lastOnline)+(Number(event.sender.getDynamicProperty("andexdbPersonalSettings:timeZone") ?? 0)*3600000)).toLocaleString()+")") + " Items: \n" + slotsArray.join("§r§f\n")))}}}catch(e){eventData.sender.sendMessage("§c" + e + e.stack)}
        ...
        case !!switchTest.match(/^einvseeb$$/):
            ...
            case !!switchTest.match(/^einvsee$$/):
                ...
                system.run(() => {
                    const inventoryd2 = world.getDimension("overworld").getEntities().concat(world.getDimension("nether").getEntities()).concat(world.getDimension("the_end").getEntities()).find((playerFinders) => (playerFinders == targetSelectorB(newMessage.slice(8), "", Number(eventData.sender.id)))).getComponent("inventory");
                    const equipmentd2 = world.getDimension("overworld").getEntities().concat(world.getDimension("nether").getEntities()).concat(world.getDimension("the_end").getEntities()).find((playerFinders) => (playerFinders == targetSelectorB(newMessage.slice(8), "", Number(eventData.sender.id)))).getComponent("equippable");
                    try {
                        let slotsArray = [];
                        if(!!inventoryd2){
                            try{
        ...
        case !!switchTest.match(/^invseeuuidmode$/):
            ...
            const equipmentd2 = world.getDimension("overworld").getEntities().concat(world.getDimension("nether").getEntities()).concat(world.getDimension("the_end").getEntities()).find((playerFinders) => (playerFinders.id == newMessage.slice(8).split(" ")[0])).getComponent("equippable");
            system.run(() => { try {
                let slotsArray = [];
                if(!!inventoryd){
                    try{
...
//            try{system.run(()=>{player.dimension.fillBlocks(evaluateCoordinates(Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[0][0], Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[1][0], Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[2][0], player.location, player.getRotation()), evaluateCoordinates(Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[3][0], Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[4][0], Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5][0], player.location, player.getRotation()), mcServer.BlockPermutation.resolve(switchTestB.split(" ").slice(1).join(" ").slice(Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5].index+Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5][0].indexOf(" ")+1).split(" ")[0], extractJSONStrings(switchTestB.split(" ").slice(1).join(" ").slice(Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5].index+Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5][0].indexOf(" ")).split(" ").slice(1).join(" "), false)[0]), {matchingBlock: mcServer.BlockPermutation.resolve(getParametersFromString(switchTestB.split(" ").slice(1).join(" ").slice(Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5].index+Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5][0].indexOf(" ")+1)).results[2], extractJSONStrings(switchTestB.split(" ").slice(1).join(" ").slice(Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5].index+Array.from(switchTestB.split(" ").slice(1).join(" ").matchAll(/\s*([\^\*\~\!][\-]?\d*|(?<![\^\*\~\!\d])[\-]?\d+)\s*/gis))[5][0].indexOf(" ")).split(" ").slice(1).join(" "), false)[1])}); }); }catch(e){eventData.sender.sendMessage("§c" + e + e.stack)}
        ...
        case !!switchTest.match(/^copyitem$/): {
            eventData.cancel = true;
            system.run(()=>{let slot = [EquipmentSlot.Head, EquipmentSlot.Chest,  EquipmentSlot.Legs, EquipmentSlot.Feet, EquipmentSlot.Mainhand, EquipmentSlot.Offhand][["head", "chest", "legs", "feet", "mainhand", "offhand", "helmet", "chestplate", "leggings", "boots", "hand", "otherhand", "cap", "tunic", "pants", "shoes", "righthand", "lefthand"].findIndex(v=>v==switchTestB.split(" ")[1]?.trim()?.toLowerCase())%6]??Number((!!!switchTestB.split(" ")[1]?.trim()?"~":switchTestB.split(" ")[1].trim()).replaceAll("~", String((world.getAllPlayers().find(_=>_.name==switchTestB.split(" ").slice(2).join(" "))??player).selectedSlot))); let toSlot = typeof slot == "string"?(world.getAllPlayers().find(_=>_.name==switchTestB.split(" ").slice(2).join(" "))??player).getComponent("equippable").getEquipmentSlot(slot):(world.getAllPlayers().find(_=>_.name==switchTestB.split(" ").slice(2).join(" "))??player).getComponent("inventory").container.getSlot(slot); toSlot.setItem(player.getComponent("inventory").container.getItem(event.sender.selectedSlot))})
        }
        ...
        case !!switchTest.match(/^swapinventories$/): {
            ...
            if(!!!target){
                ...
                system.run(()=>{inventorySwap(world.getAllPlayers().find(_=>_.name==args[1]), world.getAllPlayers().find(_=>_.name==args[2]))})
            }
        }
        break; 
        ...
        case !!switchTest.match(/^extinguish$/): {
            ...
            try{system.run(()=>{let a = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "fire"}); let b = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "soul_fire"}); player.sendMessage(`${a==0?"§c":""}${a+b} blocks extinguished in radius of ${radius}`); }); }catch(e){eventData.sender.sendMessage("§c" + e + e.stack)}
        }
        break; 
        case !!switchTest.match(/^remexp$/): {
            ...
            switch(player.dimension.id){
                case "minecraft:overworld": 
                    try{system.run(()=>{let a = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "tnt"}); let b = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "respawn_anchor"}); player.sendMessage(`${a==0?"§c":""}${a+b} explosives removed in radius of ${radius}`); }); }catch(e){eventData.sender.sendMessage("§c" + e + e.stack)}
                break; 
                case "minecraft:nether": 
                    try{system.run(()=>{let a = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "tnt"}); let b = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "bed"}); player.sendMessage(`${a==0?"§c":""}${a+b} explosives removed in radius of ${radius}`); }); }catch(e){eventData.sender.sendMessage("§c" + e + e.stack)}
                break; 
                case "minecraft:the_end": 
                    try{system.run(()=>{let a = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "tnt"}); let b = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "respawn_anchor"}); let c = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "bed"}); player.sendMessage(`${a==0?"§c":""}${a+b+c} explosives removed in radius of ${radius}`); }); }catch(e){eventData.sender.sendMessage("§c" + e + e.stack)}
                break; 
                default: 
                    try{system.run(()=>{let a = fillBlocksH(from, to, player.dimension, "air", {matchingBlock: "tnt"}); player.sendMessage(`${a==0?"§c":""}${a} explosives removed in radius of ${radius}`); }); }catch(e){eventData.sender.sendMessage("§c" + e + e.stack)}
            }
        }
        break; 
