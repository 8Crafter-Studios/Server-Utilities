...
export async function requestChatInput(player: Player, requestMessage?: string|RawMessage|(string|RawMessage)[]){
    ...
    return new Promise((resolve: (value: string) => void, reject: (error: Error) => void) => {
        function a(){if(!player.isValid()){delete currentlyRequestedChatInput[player.id]; reject(new ReferenceError("The player that the input was requested from is no longer valid, most likely the have left the game, ")); return}; if(!!!currentlyRequestedChatInput[player.id].anyInput[id].input){system.run(() => {
           a()
        })}else{let input = currentlyRequestedChatInput[player.id].anyInput[id].input; delete currentlyRequestedChatInput[player.id].anyInput[id]; resolve(input)}}
        a()
    })
}
...
export function extractIntArrayB(s: string, revivement: string ="-1"){s=s.replaceAll(/,(?=[,\]$])/g, ","+revivement); Array.from(s.matchAll(/([\-\+]?[a-zA-Z0-9]+)\^([a-zA-Z0-9]+)/g)).forEach(v=>s=s.replace(v[0], (v[1]+",").repeat(Number(v[2])).slice(0, -1))); return s}
...
export function ultraExtractIntArrayB(s: string, revivement: string ="-1"){s=s.replaceAll(/,(?=[,\]^])/g, ","+revivement); Array.from(s.matchAll(/([\-\+]?[a-zA-Z0-9]+)\^([a-zA-Z0-9]+)/g)).forEach(v=>s=s.replace(v[0], (v[1]+",").repeat(Number.parseInt(v[2], 36)).slice(0, -1))); return s}
...
export function evaluateParameters(commandstring: string, parameters: ({type: "presetText"|"number"|"boolean"|"neboolean"|"string"|"non-booleanString"|"json"|"Vector"|"Vector1"|"Vector2"|"Vector3"|"Vector4"|"Vector5"|"Vector6"|"Vector7"|"Vector8"|"targetSelector"|"blockStates", maxLength?: number}|{type: "Vectors", vectorCount?: number, maxLength?: number}|string)[]) {
