...
export class command {
    ...
    get code() { if (this.type == "custom") {
        if (this?.customCommandId != undefined) {
            return world.getDynamicPropertyIds().filter(v => v.startsWith("customCommandCode:")).sort((a, b) => Number(a?.split(":")?.slice(-1)[0]) - Number(b?.split(":")?.slice(-1)[0])).map(v => String(world.getDynamicProperty(v)));
        }
        else {
...
export class commandSettings {
    ...
    get requiredPermissionLevel() { return this?.parsed?.requiredPermissionLevel; }
