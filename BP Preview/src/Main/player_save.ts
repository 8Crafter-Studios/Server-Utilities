...
for(let i = 0; i < player.getComponent("inventory").inventorySize; i++){if (player.getComponent("inventory").container.getItem(Number(i)) !== undefined) {
    ...
    world.setDynamicProperty(savedPlayerData.saveId??`player:${savedPlayerData.id}`, JSON.stringify(savedPlayerData)); return savedPlayerData.saveId??`player:${savedPlayerData.id}`}/*
...
static getSavedPlayer(savedPlayerId: string){let playerString = String(world.getDynamicProperty(savedPlayerId)); return new savedPlayer(JSON.parse(playerString))}
